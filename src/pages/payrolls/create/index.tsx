...
function PayrollCreatePage() {
  ...
  const formik = useFormik<PayrollInterface>({
    initialValues: {
      pay_period_start: new Date(new Date().toDateString()),
      pay_period_end: new Date(new Date().toDateString()),
      gross_pay: 0,
      net_pay: 0,
      deductions: 0,
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          <FormControl id="pay_period_start" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Pay Period Start
            </FormLabel>
            <DatePicker
              selected={formik.values?.pay_period_start ? new Date(formik.values?.pay_period_start) : null}
              onChange={(value: Date) => formik.setFieldValue('pay_period_start', value)}
            />
          </FormControl>
          <FormControl id="pay_period_end" mb="4">
            <FormLabel fontSize="1rem" fontWeight={600}>
              Pay Period End
            </FormLabel>
            <DatePicker
              selected={formik.values?.pay_period_end ? new Date(formik.values?.pay_period_end) : null}
              onChange={(value: Date) => formik.setFieldValue('pay_period_end', value)}
            />
          </FormControl>
